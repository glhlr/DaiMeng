...
"""
hownet_words_list 读取(r'd:\YYY\整理.txt')，每个词转为类：HownetWords
        self.word = word  # 单词
        self.gc = gc  # 词性
        self.DEF = de  # 分类
...
# hownet文件列表
hownet_tree_files = [
    r"d:\YYY\hownet分类树\Syntax.txt",
    r"d:\YYY\hownet分类树\Secondary Feature.txt",
    r"d:\YYY\hownet分类树\Quantity数量.txt",
    r"d:\YYY\hownet分类树\Quantity Value数量值.txt",
    r"d:\YYY\hownet分类树\Event事件.txt",
    r"d:\YYY\hownet分类树\Event Role && Features.txt",
    r"d:\YYY\hownet分类树\Entity实体.txt",
    r"d:\YYY\hownet分类树\Attribute属性.txt",
    r"d:\YYY\hownet分类树\Attribute Value属性值.txt",
    r"d:\YYY\hownet分类树\Converse.txt",  # 反义词
    r"d:\YYY\hownet分类树\Antonym.txt"
]  # 反义词
...
with open(r"D:\YYY\知网-中文信息结构库.txt", 'r', encoding='UTF-8') as f:
    lines = f.readlines()
...
# 忘记做什么用了
# def stru_words_makeup(stru_x1):
#     seed(a=None, version=2)
#     stru_x = stru_x1.syn_sem_format
#     len_x = min(len(stru_x['syn']), len(stru_x['sem']))  # 元素数量不一样的，取偏少的长度。
#     example_x = ['' for i in range(len_x)]
#     syn_x = stru_x['syn']
#     sem_x = stru_x['sem']
#     for i in range(len_x):
#
#         syn_c = choice(re.split('[/]', syn_x[i])).strip()  # 用/分割
#         if syn_c == 'A':  # 把A转换为ADJ
#             syn_c = 'ADJ'
#         if syn_c.find('CLAS') != -1:
#             syn_c = 'CLAS'
#         sem_c = choice(sem_x[i])  # 从sem 的list中随机算一个词出来
#         if sem_c[0] == '“':  # 如果有“，说明就是这个词，不是分类
#             example_x[i] = sem_c[1:]
#         else:
#             if sem_c.find(
#                     '事件') != -1:  # 有事件的分类，比较难处理，如果带有多个分类，取最后一个，其他情况以后再想办法吧。
#                 words_list = find_words("", syn_c, re.split(',', sem_c)[-1])
#             else:
#                 words_list = find_words(
#                     "", syn_c,
#                     *re.split(',', sem_c)[:3])  # 如果有多个分类，就用前三个分类查询
#                 if len(words_list) == 0 and len(re.split(
#                         ',', sem_c)) > 1:  # 如果多个分类找不到结果，取最后一个分类来找
#                     words_list = find_words("", syn_c,
#                                              re.split(',', sem_c)[-1])
#
#             if len(words_list) > 0:  # 获取到多个结果，随机取一个，没有结果，则返回None
#                 example_x[i] = choice(words_list).word
#             else:
#                 example_x[i] = None
#             # 连续两个单元的都是量词的话，这两个词应该是相同的，比如一次次。
#             if i > 0 and syn_x[i].strip() == syn_x[i - 1].strip() == 'CLAS':
#                 y = example_x[i - 1]
#                 example_x[i] = y
#             # if i == 4 and syn_x[2].strip() == 'COOR':  # 处理“一遍又一遍”
#             #     example_x[4] = example_x[1]
#     if syn_x.count('NUM') == syn_x.count('CLAS') == 2:
#         example_x[syn_x.index('CLAS', 2)] = example_x[syn_x.index('CLAS', 1)]
#         example_x[syn_x.index(r'NUM',
#                               2)] = example_x[syn_x.index('CLAS', 1) - 1]
#     return stru_x1, example_x
...
def random_seek1(word='', pos=""):
    ...
    for i in range(len(word_hw.def_cn), 0, -1):
        word_stru_list = find_stru("", True, *word_hw.def_cn[:i])
        ...
        for hs_shuffle in word_hw.def_trans_groups():
            ...
            for i in reversed(stru_split):  # 按分类一级一级往上查询，找到匹配的stru后，随机取一个出来用
                word_stru_list = find_stru("", True, word_hw.gc, i)
                if len(word_stru_list) == 0:
                    pass
                else:
                    # print(i)
...
hownet_words_list = get_hownet_words(r'd:\YYY\整理.txt')
