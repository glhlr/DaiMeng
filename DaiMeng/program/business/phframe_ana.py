...
def rep_ph_ana(NLU=None, w_as=[]):  # 对话中每三句做一次分析,这里做一层控制，查找流程调用hn模块
    if not (NLU or w_as):
        return
    ph_wanas = []  # 本句所有词语列表
    N_phund = ph_und()
    if NLU:
        ...
        ph_wanas = w_as
    ana = ph_hn_ana(ph_wanas,N_phund.Semdic)   # 语义dic目前并未使用
    ana_str = ana['搭配']['思维搭配'] + '\n'
    for ff in ana['最多分类']:
        k_w = ff.split('.')[-1]
        for kk in T_dic:
...
def idiomre_ana(T_und= None, poem={}): # 诗歌重复关系分析
    if not T_und:
        return
    o_sens = re.split('[。！？，；”]', poem['原文'])
    if '标题' not in poem:
        return
    title = poem['标题']
    mean_d = {}
    omat_dic = {}
    means = hncc_ana(title)
    omat_dic['标题'] = '小呆的猜测：看标题，这诗大概讲的是，' + means[2]
    mean_d['标题'] = means[0]
    print(mean_d['标题'])
    oo = 0
    ss = 0
    mov = 0  # 正常时译文每长句两原文短句，或者一对一，一旦错位要重新定位
    for sen in T_und.phsen_ana:
        print(sen.sen_in)
        l_sen = len(re.split('[。！？，；”]', sen.sen_in))
        if oo > 0:   # 在下一句之前，解析原文，与白话文对比
            m_oo = hncc_ana(o_s)
            mean_d[o_s] = m_oo[0]  # 原文单句的内涵，两层字典
        o_s = o_sens[oo]   # 原文伴随白话文推进
        omat_dic[o_s] = {}  # 原文每小句开匹配字典来记录
        ww = 0
        bd_l = [-1]  # 标点的位置
        mt = 0
        while ww < len(sen.w_anas):
            if sen.w_anas[ww].wo in '《》':
                ww += 1
                continue
            n_dic = s_w_ana(sen.w_anas[ww], o_s)
            if n_dic:
                omat_dic[o_s].update(s_w_ana(sen.w_anas[ww], o_s))   # 词对词的匹配
            if sen.w_anas[ww].pos == '标点':
                bd_l.append(ww)  # 记录标点的序号
                if len(bd_l)-1 < int(l_sen/2):
                    ww += 1
                    continue
                for k in omat_dic[o_s]:
                    mt += len(k)
                    if len(k) == 1 and k == omat_dic[o_s][k]:
                        mt += 1
                if mt > int((len(o_s)-1)/2):
                    ss_in = ''
                    for wi in range(bd_l[0] + 1, bd_l[-1]):
                        ss_in += sen.w_anas[wi].wo
                    omat_dic[o_s][o_s] = ss_in
                    if ss > 0:
                        for fw in T_und.phsen_ana[ss-1].w_anas:
                            if fw.pos in '名词动词形容词副词':   # 分别确认原-白话对应的词
                                r_l = s_w_ana(fw,o_s)
                                if '关联' not in omat_dic[o_s]:
                                    omat_dic[o_s]['关联'] = r_l
                                else:
                                    omat_dic[o_s]['关联'].update(r_l)
                    mov = 0

                elif mt < 2:
                    mov += 1
                    if mov > 1:   # 第一次匹配不上 ，无视继续；第二次匹配不上，退o_s匹配；第三次
                        bd_l = [bd_l[0]]
                        ww = bd_l[0] + 1
                        oo += -1
                        o_s = o_sens[oo]
                        ...
                        continue
                mt = 0
                if len(bd_l) > 2 and len(bd_l)-1 == l_sen/2: # 译文句子中间的一句公用。
                        ww = bd_l[-2]
                        bd_l = [bd_l[-2]]
                else:
                    bd_l = [bd_l[-1]]
    ...
    for ii in range(0,len(o_sens),4):
        t_sens = o_sens[ii:min(len(o_sens), ii+4)]
        if '' in t_sens:
            t_sens.remove('')
        mean_t = {}
        for tt in t_sens:   # 对于长诗，步长四句截取一段匹配经验，注意步长与匹配长度不一致，部分重复
            mean_t[tt] = mean_d[tt]
        Csmat = Cs_ana(t_sens, mean_t)
        if not Csmat:
            continue
        for cs in Csmat:
            if Csmat[cs]['ele_mat'] <= len(Csmat[cs])/2 -1:
                if cs not in last_dic:
                    continue
                for lc in last_dic[cs]:
                    if not last_dic[cs][lc]:
                        continue
                    if lc == 'ele_mat':
                        continue
                    if Csmat[cs][lc]:
                        ...
                        Csmat[cs][lc] = last_dic[cs][lc]
                        Csmat[cs]['ele_mat'] += 1
            if Csmat[cs]['ele_mat'] > len(Csmat[cs]) / 2 - 1:
                if cs not in Cs_talk:
                    Cs_talk[cs] = '小萌琢磨:'
                if Cs_talk[cs].find('这是一次' + cs) > -1:
                    print(Csmat[cs])
                    w_r0 = ''    # 匹配上的短句
                    for mk in Csmat[cs]:
                        if mk ==  'ele_mat':
                            continue
                        if Csmat[cs][mk]:
                            if Cs_talk[cs].find(Csmat[cs][mk][0]) == -1:
                                if w_r0.find(Csmat[cs][mk][0]) == -1:
                                    w_r0 += Csmat[cs][mk][0] + '，'
                    w_r1 = T_dic['T.'+cs].split('+')  # 主题描述
                    w_r = T_dic['T.继续'].split('+')  # 对继续的描述
                    Cs_talk[cs] += '小呆跟进:后边还有' + w_r0  + cs + w_r[random.randint(0, len(w_r) - 1)] + ',' + w_r1[random.randint(0, len(w_r1) - 1)] +'\n'
                    continue
                spe = ''
                for cs_e in Csmat[cs]:
                    if cs_e in ['ele_mat']:
                        continue
                    spg = '小萌:我猜还可能讲'
                    if Csmat[cs][cs_e]:   # 描述这一句有哪个元素
                        for item in cs_e.split(','):
                            if Csmat[cs][cs_e][1].split('.')[-1] not in spe:
                                spe += '、' + Csmat[cs][cs_e][1].split('.')[-1]
                        spe = spe.strip('、')
                        Cs_talk[cs] += Csmat[cs][cs_e][0] + ','
                    else:   # 如果没发现，表示还有猜测
                        for item in cs_e.split(','):
                            spg += item.split('.')[0] + '、'
                        spg = spg.strip('、')
                ...
                if 'T.'+cs in T_dic:
                    w_r2 = T_dic['T.'+cs].split('+')
                    Cs_talk[cs] +=  '小萌:所以，这是一次' + cs + '，' + spg + ','+ w_r2[random.randint(0, len(w_r2) - 1)] +'。\n'
        last_dic = {}
    for cs in Cs_talk:
        trans_ltp.thinking_str += Cs_talk[cs]
        print(Cs_talk[cs])
    # print(omat_dic)  # 返回原文与译文词语对应的字典
    return(omat_dic)


def s_w_ana(w= None, o_s = ''):   # 一个w_ana与短句string的基础匹配，以后更复杂的关联匹配也放在这里
    if w is None or o_s == '':
        return
    o_dic = {}
    wo = w.wo
...
def Cs_ana(o_sen=[], mean_d={}):
    if mean_d == {} or o_sen == []:
        return
    cm_dic = {}
    for cs in Cs_dic:  # 单条的常识
        cm_dic[cs] = {}
        ele_mat = 0
        for cs_e in Cs_dic[cs].split('|'):  # 常识的各元素
            s_mat = False
            for o_s in o_sen:  # 原诗短句
                if s_mat:
                    break
                if o_s not in mean_d:
                    continue
                for ms_e in mean_d[o_s]:  # 短句的内涵字典
                    cm_dic[cs][cs_e] = csele_mat(cs_e,ms_e + '.' + mean_d[o_s][ms_e])
                    if cm_dic[cs][cs_e]:
                        cm_dic[cs][cs_e] = [o_s, ms_e, mean_d[o_s][ms_e]]  # 如果匹配成功，送出诗歌原句、内涵树、内涵树的所在的词
                        s_mat = True
                        ele_mat += 1
                        break
        if ele_mat > 0:
            cm_dic[cs]['ele_mat'] = ele_mat
        else:
            del cm_dic[cs]
    return cm_dic
    pass
...
def csele_mat(cs_e='', ms_e=''):  # cs元素匹配，准备用于复杂变量
    if cs_e == '' or ms_e == '':
        return False
    if_m = False
    sp_mse = re.split(r'[+\.]', ms_e)
    sp_cse = re.split(r'[,\.]', cs_e)


    for s_m in sp_mse:
        if s_m not in sp_cse:
            if len(s_m) < 2:  # 两个字符都在同义词中，也可行
                continue
            if (s_m[0] not in sp_cse) or (s_m[-1] not in sp_cse):
                continue
        if_m = True
        break
    return if_m
