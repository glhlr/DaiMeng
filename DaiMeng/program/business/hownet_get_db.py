    ...
    for kk in rel_gn:
        ...
        for se in rel_gn[kk]:
            ...
            for ee in se.def_cn:
                if not ee:
                    continue
                if ee in ['属性','属性值','数量值']:  # 属性值是多余信息，后边的意思足够
                    continue
                if ee in ['文字']:  # 对姓氏，先去掉
                    break
                if ee[0] in s_dic:
                    ee = ee[1:]
                if len(rel_gn[kk]) > 2:
                    if ee not in Tr_und_dic:
                        continue
                if ee not in m_d:
                    m_d[ee] = kk
                elif kk not in m_d[ee].split('+'):
                    m_d[ee] += '+' + kk
                tal += ee
            ...
            if len(rel_gn[kk]) == 1:
                te = se.def_cn
                while te[0] in ['属性','属性值','数量值']:  # 这里也不需要属性
                    te.remove(te[0])
                tot[-1] += (kk + '这' + se.def_cn[0])

		
                kkk = se.def_trans_groups()
                if not kkk:
                    continue
