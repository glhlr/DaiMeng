...
import business.phase_json as phase_json
...
def rep_talk(input='我们出门买橡皮呗'):
    ...
    if len(test_dic['回复']) < 3:
        ...
        for key in ph_und.Semdic:
            if key == 'V':
                for vv in ph_und.Semdic[key].split(','):
                    if vv.find('sn=')>-1:
                        continue
                    trans_ltp.thinking_str += '重要的' + vv + '\n'
	
        print('thinking::',trans_ltp.thinking_str)
        test_dic['回复'] += Thlog_ana(trans_ltp.thinking_str)
