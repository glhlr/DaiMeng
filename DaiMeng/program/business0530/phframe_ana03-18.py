...
def concept_sen_match(this_und, tag, sen_type):
    ...
    #tree = ET.parse(r'D:/自知者SVN/DaiMeng/DaiMeng/data/xmlData/表达2.xml')
    root = tree.getroot()
    sentence_list = []
...
def phase_ana():
    ...
    if len(this_und.phsen_ana) <2:
        return
    p_xml = r'/home/ubuntu/DaiMeng/data/xmlData/'
    #p_xml = r'D:/自知者SVN/DaiMeng/DaiMeng/data/xmlData/'
    fs = os.listdir(p_xml)
    files = []
    ana_no = []
    for f1 in fs:
                                ...
                                if('{' in process_node):
                                                ...
                                                if(qa_sub_node.attrib['主题'] and (qa_sub_node.attrib['主题'] == sub_proc)):
                                                    ...
                                                    if(qa_sub_node.tag.find('Q_A关键字') > -1):
                                                        ...
                                                        for qa in qa_key_lst:
                                                            if ('@N.' in qa):
                                                                qa_str = qa[qa.find('.') + 1:]
                                                                qa_key += sta_no.text[sta_no.text.find(qa_str)+len(qa_str)+1:sta_no.text.find('；',sta_no.text.find(qa_str))] + '+'
                                                            if ('@C.' in qa):
                                                                qa_key += qa[qa.find('.')+1:] + '+'
                                                        qa_key = qa_key[:-1]
                                    ...
                                    for prop_str in prop_lst:
                                        if (sub_proc in prop_str):
                                            ...
                                            if (len(main_sub) > 0 and len(question) > 0):
                                                trans_ltp.thinking_str += '小呆回答问题:' + main_sub + prop_str[prop_str.find('=')+1:] + '呀\n'
                                            break
                ...
                if chi.find('内涵') == None:
                    ...
                    for fra_item in fra_node:
                        ...
                        if not flag:
                            break
                        if sta_no != None:
                            for sub_node in sta_no:
                                if (sub_node.tag.find('主体') > -1):
                                    # 找到<**主体>标签，根据主体模板替换掉主语内容
                                    sub_node.text = subject_sts(this_und, sub_node.text)
                                if (sub_node.tag.find('属性') > -1):
                                    # 找到<过程_属性>标签，填上属性内容，包括距离、方式
                                    for sub_sub_node in sub_node:
                                        for prop_str in prop_lst:
                                            if (sub_sub_node.tag in prop_str):
                                                sub_sub_node.text = prop_str[prop_str.find('=') + 1:]
                                                break
                                if (sub_node.tag.find('Q_A关键字') > -1):
                                    # 找到<Q_A关键字>标签，根据之前查询的主体、属性填上查询关键字
                                    qa_lst = sub_node.text.split('+')
                                    sub_node.text = ''
                                    for qa in qa_lst:
                                        if ('@N.' in qa):
                                            qa_str = qa[qa.find('.') + 1:]
                                            for qa_node_search in sta_no.iter(qa_str):
                                                if (qa_node_search.text):
                                                    sub_node.text += qa_node_search.text + '+'
                                        if ('@C.' in qa):
                                            sub_node.text += qa[qa.find('.') + 1:] + '+'
                                    sub_node.text = sub_node.text[:-1]
                                    search_str = sub_node.text.replace('+', ' ')
                                    print('search_str: ', search_str)
                                    result_str = Query(search_str)['Answer']  # 按关键字爬取段落并返回
                                    print('result_str: ', result_str)
                                    result_str.strip('\r\n').replace(u'\u3000', ' ').replace(u'\xa0', u' ')
                                if (sub_node.tag.find('Q_A匹配') > -1):
                                    for qa_node in sub_node.iter('句式'):
                                        for childnode in qa_node.iter('描述句式'):
                                            # 取出搜做的主题词，如果没有主题词则用匹配到的逻辑句式主题直接替代
                                            if (qa_node.attrib['主题']):
                                                key_str = qa_node.attrib['主题']
                                            else:
                                                key_str = qa_node.find('逻辑句式').text[
                                                          :qa_node.find('逻辑句式').text.find('=')]

                                            try:
                                                logic_result_lst = spider_result(result_str, childnode.text,qa_node.find('逻辑句式').text)
                                            except Exception:
                                                print('qa_no空了')
                                                continue
                                            print(logic_result_lst)
                                            if (logic_result_lst):
                                                for logic_result in logic_result_lst:
                                                    if (logic_result):
                                                        c_sta = SubElement(sta_no, key_str)
                                                        c_sta.text = logic_result[0].replace('+', '')
                                                        c_sta.tail = '\n\t'
                            # 以上为工添加
                            ...
                            if len(frsem_dic) > 1:
                                for k_sn in frsem_dic:
                                    ls = len(sta_no[1].tag)
                                    c_sta = SubElement(sta_no, sta_no[1].tag[:ls-1])
                                    c_sta.text = frsem_dic[k_sn]
                                    c_sta.tail = '\n\t'
                                ana_no.append(sta_no)

    wr_ph(ana_no=ana_no)
def wr_ph( file=r'/home/ubuntu/DaiMeng/data/xmlData/语义.段落.xml', ana_no = None):      #写入指定文件，在阅读场景模板中插入扩展思维模板节点
 
#def wr_ph( file=r'D:/自知者SVN/DaiMeng/DaiMeng/data/xmlData/语义.段落.xml', ana_no = None):      #写入指定文件，在阅读场景模板中插入扩展思维模板节点
    tree0 = ET.parse(file)
    root = tree0.getroot()
    global ph
...
def pron_indicate(this_und):
# 代词指代内容替换
    pron_lst = []
    for i, row in enumerate(this_und.phsen_ana):
        pron_str = 'sn='+str(i)
        for j, w_ana in enumerate(row.w_anas):
            current_sign = 1  # 当前句标识，当前句才判断是否有“说”字，如果向上一句查找则不再看“说”字
            if(w_ana.pos == '代词'):
                if(w_ana.wo == '我' or w_ana.wo == '自己' or w_ana.wo == '本人'):
                    ii = i
                    while(ii>=0):
                        sign_index = -1 #“说”前为我
                        jj = 0
                        # 找到“说”字的位置
                        while(jj<len(row.w_anas) and current_sign == 1):
                            if(this_und.phsen_ana[ii].w_anas[jj].wo == '说' or this_und.phsen_ana[ii].w_anas[jj].wo == '问'):
                                sign_index = jj
                                break
                            jj += 1
                        sign_index -= 1
                        # 从“说”字往前查找
                        while(sign_index>=0):
                            if((this_und.phsen_ana[ii].w_anas[sign_index].sp_cla == '主语'
                                    or this_und.phsen_ana[ii].w_anas[sign_index].sp_cla == '宾语')
                                    and this_und.phsen_ana[ii].w_anas[sign_index].pos != '代词'
                                    and this_und.phsen_ana[ii].w_anas[sign_index].pos != '动词'
                                    and this_und.phsen_ana[ii].w_anas[sign_index].cla.find('人造物') < 0
                                    and (this_und.phsen_ana[ii].w_anas[sign_index].cla.find('人')>=0
                                        or this_und.phsen_ana[ii].w_anas[sign_index].cla.find('动物')>=0)):
                                pron_str = pron_str + ',' + this_und.phsen_ana[i].w_anas[j].wo+'='+\
                                           this_und.phsen_ana[ii].w_anas[sign_index].wo
                                ii = -1
                                break
                            sign_index -= 1
                        ...
                        while (jj<len(this_und.phsen_ana[ii].w_anas) and sign_index < -1 and current_sign == 0):
                            if ((this_und.phsen_ana[ii].w_anas[jj].sp_cla == '主语'
                                    or this_und.phsen_ana[ii].w_anas[jj].sp_cla == '宾语')
                                    and this_und.phsen_ana[ii].w_anas[jj].pos != '代词'
                                    and this_und.phsen_ana[ii].w_anas[jj].pos != '动词'
                                    and this_und.phsen_ana[ii].w_anas[jj].cla.find('人造物') < 0
                                    and (this_und.phsen_ana[ii].w_anas[jj].cla.find('人') >= 0
                                         or this_und.phsen_ana[ii].w_anas[jj].cla.find('动物') >= 0)):
                                pron_str = pron_str + ',' + this_und.phsen_ana[i].w_anas[j].wo + '=' + \
                                           this_und.phsen_ana[ii].w_anas[jj].wo
                                ii = -1
                                break
                            jj += 1
                        ii -= 1
                elif (w_ana.wo == '你' or w_ana.wo == '您'):
                    ii = i
                    while (ii >= 0):
                        sign_index = len(row.w_anas)  # “说”后为你
                        jj = 0
                        # 找出“说”字的位置
                        while (jj < len(row.w_anas) and current_sign == 1):
                            if (this_und.phsen_ana[ii].w_anas[jj].wo == '说'
                                    or this_und.phsen_ana[ii].w_anas[jj].wo == '问'):
                                sign_index = jj
                                break
                            jj += 1
                        sign_index += 1
                        # 从“说”往后查找
                        while (sign_index < len(row.w_anas)):
                            if ((this_und.phsen_ana[ii].w_anas[sign_index].sp_cla == '主语'
                                    or this_und.phsen_ana[ii].w_anas[sign_index].sp_cla == '宾语')
                                    and this_und.phsen_ana[ii].w_anas[sign_index].pos != '代词'
                                    and this_und.phsen_ana[ii].w_anas[sign_index].pos != '动词'
                        ...
                        if(sign_index>len(row.w_anas) and current_sign == 1):
                            sign_index = j-1
                            while(sign_index >= 0 and current_sign == 1):
                                if ((this_und.phsen_ana[ii].w_anas[sign_index].sp_cla == '主语'
                                     or this_und.phsen_ana[ii].w_anas[sign_index].sp_cla == '宾语')
                        ...
                        while (jj < len(this_und.phsen_ana[ii].w_anas) and sign_index > len(row.w_anas) and current_sign == 0):
                            if ((this_und.phsen_ana[ii].w_anas[jj].sp_cla == '主语'
                                    or this_und.phsen_ana[ii].w_anas[jj].sp_cla == '宾语')
                                    and this_und.phsen_ana[ii].w_anas[jj].pos != '代词'
                                    and this_und.phsen_ana[ii].w_anas[jj].pos != '动词'
                            ...
                            jj += 1
                        ii -= 1
                elif(w_ana.wo == '他' or w_ana.wo == '她' or w_ana.wo == '它'):
                    ...
                    while (ii >= 0):
                        ...
                        while (jj < len(this_und.phsen_ana[ii].w_anas) and sign_index < -1 and current_sign == 0):
                            ...
                            jj += 1
                        ii -= 1
                elif(w_ana.wo == '我们' or w_ana.wo == '你们' or w_ana.wo == '他们' or w_ana.wo == '她们' or w_ana.wo == '它们'):
                    # 当前句从代词开始往前搜，前一句从第一个词开始往后搜
                    ii = i
                    ...
                    while (ii >= 0):
                        ...
                        while (sign_index >= 0 and current_sign == 1):
                            if ((this_und.phsen_ana[ii].w_anas[sign_index].sp_cla == '主语'
                                or this_und.phsen_ana[ii].w_anas[sign_index].sp_cla == '并列'
                                or this_und.phsen_ana[ii].w_anas[sign_index].sp_cla == '宾语')
                                    and this_und.phsen_ana[ii].w_anas[sign_index].pos != '代词'
                                    and this_und.phsen_ana[ii].w_anas[sign_index].pos.find('名') >=0
                                    and this_und.phsen_ana[ii].w_anas[sign_index].cla.find('人造物') < 0
                                    and (this_und.phsen_ana[ii].w_anas[sign_index].cla.find('人') >= 0
                                         or this_und.phsen_ana[ii].w_anas[sign_index].cla.find('动物') >= 0)):
                                if(words == ''):
                                    words = this_und.phsen_ana[ii].w_anas[sign_index].wo
                                else:
                                    words = words + '+' + this_und.phsen_ana[ii].w_anas[sign_index].wo
                                pron_count += 1
                                if(pron_count >=2 ):
                                    pron_str = pron_str + ',' + this_und.phsen_ana[i].w_anas[j].wo + '=' + words
                                    break
                            sign_index -= 1
                        jj = 0
                        current_sign = 0
                        ...
                        while (jj < len(this_und.phsen_ana[ii].w_anas) and sign_index < -1 and current_sign == 0):
                            if ((this_und.phsen_ana[ii].w_anas[jj].sp_cla == '主语'
                                or this_und.phsen_ana[ii].w_anas[jj].sp_cla == '并列'
                                or this_und.phsen_ana[ii].w_anas[jj].sp_cla == '宾语')
                                    and this_und.phsen_ana[ii].w_anas[jj].pos != '代词'
                                    and this_und.phsen_ana[ii].w_anas[jj].cla.find('人造物') < 0
                                    and this_und.phsen_ana[ii].w_anas[jj].pos.find('名') >= 0
                                    and (this_und.phsen_ana[ii].w_anas[jj].cla.find('人') >= 0
                                         or this_und.phsen_ana[ii].w_anas[jj].cla.find('动物') >= 0)):
                                if(words == ''):
                                    words = this_und.phsen_ana[ii].w_anas[jj].wo
                                else:
                                    words = words + '+' + this_und.phsen_ana[ii].w_anas[jj].wo
                                pron_count += 1
                                if(pron_count >= 2):
                                    pron_str = pron_str + ',' + this_und.phsen_ana[i].w_anas[j].wo + '=' + words
                                    break
