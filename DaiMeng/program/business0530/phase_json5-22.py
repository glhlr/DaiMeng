...
def auto_express(logic_str, logic_lst):
    ...
    express = express_lst[random.randint(0, len(express_lst) - 1)]
    i = 0
...
def logic_estimate(sen_ana, sub_sen='', cognize_str='', subsenana_lst = [], sn=0):
    ...
    logic_metaphor=['像','就像','好像','好似','仿佛'] # 比喻
    for w_ana in sen_ana.w_anas:
        # print(w_ana.wo+': ')
        if(len(sub_sen)>0):
            if(w_ana.wo not in sub_sen):
    ...
    if(sub_sen=='' and logic_dic['比喻']>0 ):
        ...
        if(len(ontology)>0 and len(vehicle)>0):
            ...
            trans_ltp.thinking_str += '小呆陷入沉思:' + '发现比喻句式，将' + ontology + '比喻为' + vehicle +'\n'
            logic_str = '比喻：' + '将' + ontology + '比喻为' + vehicle
        else:
            # ontology = ''
            # vehicle = ''
...
def phase_json_ana(input_str, idiom = True):
    ...
    ana_lst = []
    idiomre_ana(this_und, poem_dic['原文'])
