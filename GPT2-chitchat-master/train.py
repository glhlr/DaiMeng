...
def setup_train_args():
    ...
    parser.add_argument('--train_raw_path', default='data/sumtrain00.txt', type=str, required=False, help='原始训练语料')
    ...
    parser.add_argument('--dialogue_model_output_path', default='dialogue_model/', type=str, required=False,
                        help='对话模型输出路径')
    parser.add_argument('--pretrained_model', default='dialogue_model/epoch1', type=str, required=False, help='预训练的GPT2模型的路径')
    parser.add_argument('--writer_dir', default='tensorboard_summary/', type=str, required=False, help='Tensorboard路径')
    parser.add_argument('--seed', type=int, default=None, help='设置种子用于生成随机数，以使得训练的结果是确定的')
    parser.add_argument('--num_workers', type=int, default=16, help="dataloader加载数据时使用的线程数量")
...
def main():
    ...
    if args.seed:
        set_random_seed(args).to(device)
