...
def evaluate(exe,
    ...
    mrc_result=write_predictions(examples, features, all_results, args.n_best_size,
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        ii = 0  #以下调整正确把握的评分，对长答案扣分
        for output in nbest_json[0:int(len(nbest_json)/3)]:
            if len(output["text"]) > 11:   # 12字以上，每多两字扣1%
                if output["text"].find("、") > -1:
                    continue
                nbest_json[ii]["probability"] = output["probability"] * (100-(len(output["text"])-12)/2)/100
            ii += 1
        nbest_json = sorted(nbest_json,key=lambda x: (x["probability"]),    # 重新按可能性排序
            reverse=True)

        #print('我找到哪里直接print结果了！！！',nbest_json[0]["text"])
        #print('', nbest_json)
    ...
    mrc_result = (nbest_json[0],nbest_json)
    return mrc_result
