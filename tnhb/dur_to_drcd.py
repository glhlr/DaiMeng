...
class du_zhidao(object):

    def select_zhidao():  # 度鱼四分类改造
                ...
                if case['question'] != act_qu:
                    ...
                    for o_q in Duodata: # 老度鱼数据
                        if o_q == case['question']:
                            if_m = True
                            break
                        elif len(s_c(o_q,case['question']))/len(case['question']) > 0.9:
                            if_m = True
                            break
                if if_m:
                    continue
        ...
        print(len(duz_save))
        with open('du_sear.json', 'w', encoding='UTF-8') as f1:
            f1.write(duz_save)
            f1.close()
...
class du_to_drcd(object):
    ...
    def make_drcd():
        ...
        with open('/home/lxz/阅读理解比赛/个人赛数据集/train.json', 'r', encoding='UTF-8') as f1:
            ...
            for line in lines:
                ...
                if max_sa > 0.75:
                    ...
                    para_dic['context'] += '。该问题没有答案。观点选项有：是的，不是，看情况。'
                    js[0] += 1
        ...
        with open('dutodr_zhidao.tran.json', 'w', encoding='UTF-8') as f3:
            json.dump(Dr_dic, f3, ensure_ascii=False, indent=1)
            f3.close()
        print(ii,js)


#du_to_du4.read_txt("e:/个人赛数据集/train.json")
#du_to_du4.words_dusts()
# du_to_du4.wm_dusts()
# du_to_du4.test11k()
# du_to_drcd.make_drcd()

du_zhidao.select_zhidao()